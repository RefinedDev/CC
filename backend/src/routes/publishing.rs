...
async fn list(
    ...
    let claims = auth::auth_from_headers(&headers).map_err(|_| (StatusCode::UNAUTHORIZED, Json(serde_json::json!({"message":"Invalid or missing authentication token."}))))?;
    Ok(Json(db::list_publications(query.kind.as_deref(), &claims.sub).map_err(
        |_| {
            (
...
async fn create(
    ...
    let target_user_id = payload.get("target_user_id").and_then(|v| v.as_str()).map(str::trim).filter(|v| !v.is_empty());
    if let Some(user_id) = target_user_id {
        if db::find_user_by_id(user_id).map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"message":"Failed to validate target user."}))))?.is_none() {
            return Err((StatusCode::BAD_REQUEST, Json(serde_json::json!({"message":"Target user was not found."}))));
        }
    }
    let item = db::insert_publication(kind, title, body, &claims.sub, target_user_id).map_err(|_| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
...
async fn mark_read(
    ...
    let claims = auth::auth_from_headers(&headers).map_err(|_| (StatusCode::UNAUTHORIZED, Json(serde_json::json!({"message":"Invalid or missing authentication token."}))))?;
    db::mark_publication_read(id, &claims.sub).map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"message":"Failed to mark notification as read."}))))?;
...
async fn mark_all_read(
    ...
    let claims = auth::auth_from_headers(&headers).map_err(|_| (StatusCode::UNAUTHORIZED, Json(serde_json::json!({"message":"Invalid or missing authentication token."}))))?;
    db::mark_all_publications_read(&claims.sub).map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"message":"Failed to mark notifications as read."}))))?;
    Ok(StatusCode::NO_CONTENT)
}
