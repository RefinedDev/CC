(function () {
  document.addEventListener('DOMContentLoaded', function () {
    ...
    window.CapacityApi.get('/assessments').then(items => {
      list.innerHTML = items.length ? items.map(item => {
        const result = item.result ? `<small>Score: ${item.result.score}/${item.result.total}</small>` : `<a class="btn btn-primary" href="assessment.html?id=${item.id}">Start</a>`;
      ...
      }).join('') : '<p>No assessments are available yet.</p>';
