(function () {
  ...
  async function loadCourses() {
    ...
    courseSelect.innerHTML = courses.filter(course => {
  ...
  async function loadLectures() {
    ...
    const lectures = await window.CapacityApi.get(`/courses/${encodeURIComponent(selectedCourse)}/lectures`);
    ...
    lectureList.querySelectorAll('[data-delete-lecture]').forEach(button => {
      button.onclick = async () => {
        await window.CapacityApi.delete(`/courses/${encodeURIComponent(selectedCourse)}/lectures/${button.dataset.deleteLecture}`);
        await loadLectures();
      };
    });
  }
  ...
  courseSelect.addEventListener('change', () => {
    selectedCourse = courseSelect.value;
    loadLectures().catch(error => { lectureMessage.textContent = error.message; });
  });
  lectureForm.addEventListener('submit', async event => {
    event.preventDefault();
    try {
      await window.CapacityApi.post(`/courses/${encodeURIComponent(selectedCourse)}/lectures`, {
  ...
  loadCourses().catch(error => { lectureMessage.textContent = error.message || 'Unable to load courses.'; });
})();
