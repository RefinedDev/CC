(function () {
  ...
  Promise.all([
    ...
    .then(async ([courses, resources, assessments, publications]) => {
      ...
      document.getElementById("traineeCourseCount").textContent =
        enrolled.length;
      ...
      document.getElementById("traineeAssessmentCount").textContent =
        assessments.length;
      document.getElementById("traineeAssessmentDetail").textContent =
      ...
      resourceList.innerHTML = resources.length
        ? resources
            ...
            .join("")
        : '<p>No resources uploaded yet. <a href="library.html">Browse the library</a>.</p>';
      document.getElementById("traineeActivity").innerHTML = publications.length
        ? publications
            .slice(0, 4)
            .map(
              (item) =>
                `<div>🔔 <span>${escapeText(item.title)}<small>${escapeText(item.kind)} · ${escapeText(item.created_at)}</small></span></div>`,
            )
            .join("")
      ...
      const completedCourses = courseData.filter(
        (item) =>
          item.lectures.length > 0 && item.completed === item.lectures.length,
      );
      const lessonsCompleted = courseData.reduce(
        (total, item) => total + item.completed,
        0,
      );
      document.getElementById("traineeCertificateCount").textContent =
        completedCourses.length;
      document.getElementById("traineeLessonCount").textContent =
        lessonsCompleted;
      ...
      if (assessmentResults.length >= 5)
        earned.push([
          "⭐",
          "Quick Learner",
          `Completed ${assessmentResults.length} assessments`,
        ]);
      ...
      const highScores = assessmentResults.filter(
        (item) => item.result.score / item.result.total >= 0.9,
      ).length;
      if (highScores >= 3)
        earned.push([
          "🎯",
          "Top Performer",
          `Scored 90%+ in ${highScores} assessments`,
        ]);
      ...
      if (!courseData.length) {
        target.innerHTML =
          '<p>You are not enrolled in any courses yet. <a href="course-catalog.html">Browse courses</a>.</p>';
        return;
      }
      const current =
        courseData.find((item) => item.percent < 100) || courseData[0];
