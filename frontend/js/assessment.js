(function () {
  ...
  function render() {
    ...
    document.getElementById("qprogress").style.width =
      `${((current + 1) / assessment.questions.length) * 100}%`;
    document.querySelectorAll("#options input").forEach(
      (input) =>
        (input.onchange = () => {
  ...
  function renderGrid() {
    document.getElementById("qgrid").innerHTML = assessment.questions
      .map(
        (question, index) =>
          `<button class="${index === current ? "current" : ""} ${answers[question.id] ? "answered" : ""}" onclick="window.goQuestion(${index})">${index + 1}</button>`,
      )
      .join("");
  }
  window.goQuestion = (index) => {
    current = index;
    render();
  };
  window.prevQ = () => {
    if (current > 0) window.goQuestion(current - 1);
  };
  window.nextQ = () => {
    if (!answers[assessment.questions[current].id])
      return window.showToast("Please select an answer first.");
    if (current < assessment.questions.length - 1)
      window.goQuestion(current + 1);
    else window.submitAssessment();
  };
  window.submitAssessment = async () => {
    if (
      ...
      return;
    try {
      const result = await window.CapacityApi.post(
        `/assessments/${assessment.id}/attempt`,
        { answers },
      );
      location.assign(
        `assessment-result.html?id=${assessment.id}&score=${result.score}&total=${result.total}`,
      );
    } catch (error) {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
      assessment = await window.CapacityApi.get(`/assessments/${id}`);
      document.title = `${assessment.title} | Capacity Connect`;
      render();
      startTimer();
    } catch (error) {
      document.getElementById("question").textContent = error.message;
    }
  });
})();
