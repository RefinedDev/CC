(function () {
  ...
  function render() {
    ...
    document.getElementById('lessonBadge').textContent = `${total} LESSON${total === 1 ? '' : 'S'}`;

    lectureList.innerHTML = lessons.map((lecture, index) => {
  ...
  window.watchLecture = function (index) {
    ...
    document.getElementById('completeBtn').onclick = function () {
      window.CapacityApi.post(`/courses/${encodeURIComponent(courseId)}/progress/${lessons[index].id}`)
        .then(() => {
        ...
        .catch(error => {
          document.getElementById('watchTitle').textContent = error.message || 'Unable to save progress.';
        });
    };
    render();
  };
  ...
  Promise.all([
    ...
    window.CapacityApi.get(`/courses/${encodeURIComponent(courseId)}/progress`)
      ...
      progress.completedLectures = courseProgress.completed_lectures.map(lectureId =>
