(function () {
  ...
  Promise.all([
    window.CapacityApi.get("/courses/analytics"),
    window.CapacityApi.get("/resources"),
  ])
    .then(([analytics, resources]) => {
      const courses = analytics.courses;
      document.getElementById("trainerCourseCount").textContent =
        courses.length;
      document.getElementById("trainerAssessmentCount").textContent =
        analytics.assessments.length;
      document.getElementById("trainerResourceCount").textContent =
        resources.filter((resource) => resource.created_by === user.id).length;
      const scores = analytics.assessments.filter((item) => item.attempts > 0);
      document.getElementById("trainerAverageScore").textContent = scores.length
        ? `${Math.round(scores.reduce((sum, item) => sum + item.average_score, 0) / scores.length)}%`
        : "—";
      list.innerHTML = courses.length
        ? courses
            ...
            .join("")
        : "<p>No courses created yet. Use Manage Courses to create one.</p>";
      const chart = document.getElementById("trainerCourseChart");
      chart.innerHTML = courses.length
        ? courses
            .map(
              (course) =>
                `<i style="height:${Math.max(5, course.completion_rate)}%"><span>${escapeText(course.title.slice(0, 8))}</span></i>`,
            )
            .join("")
        : "<p>No course analytics available.</p>";
      const assessmentList = document.getElementById(
        "trainerAssessmentAnalytics",
      );
      assessmentList.innerHTML = analytics.assessments.length
      ...
      traineeList.innerHTML = analytics.trainees.length
        ? analytics.trainees
            .map(
              (item) =>
            ...
            .join("")
        : "<p>No enrolled trainees yet.</p>";
      const attempts = document.getElementById("trainerAssessmentAttempts");
      attempts.innerHTML = analytics.attempts.length
        ? analytics.attempts
            .map(
              (item) =>
