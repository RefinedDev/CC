  ...
  async function loadUsers() {
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px">
        ...
        <select data-user-id="${escapeText(user.id)}">
          ${['trainee', 'trainer', 'admin'].map(role =>
            `<option value="${role}" ${role === user.role ? 'selected' : ''}>${role}</option>`).join('')}
        </select>
      </div>
    `).join('');
    list.querySelectorAll('select[data-user-id]').forEach(select => {
      select.onchange = async () => {
        try {
          await window.CapacityApi.post(`/admin/users/${encodeURIComponent(select.dataset.userId)}/role`, { role: select.value });
